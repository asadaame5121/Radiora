...
<section class="help-panel" aria-labelledby="help-title">
	...
	<div class="help-grid" aria-label="ヘルプのトピック">
		...
		<article class="help-card">
			...
			<h2>日々の確認と長文執筆</h2>
			<ul>
				<li>「今日」ビューで期間を選び、その期間に作成・更新した項目を分けて確認できます。</li>
				<li>重要な項目には「栞」を挟んで素早くアクセスできます。</li>
				<li>最後に作業していた「再開位置」は自動保持され、作業をスムーズに再開できます。</li>
				<li>選択項目のインスペクター「概要」で「原稿として開く」を選ぶと、長文編集モードが中央ペインに展開します。編集とプレビューを切り替え、保存またはキャンセルできます。</li>
				<li><kbd>Enter</kbd>で兄弟、<kbd>Shift</kbd> + <kbd>Enter</kbd>で本文中の改行を入力します。</li>
				<li><kbd>Tab</kbd> / <kbd>Shift</kbd> + <kbd>Tab</kbd>で階層を移動できます。</li>
				<li>右側のインスペクターは「概要・関係・履歴」を切り替えます。「概要」には配置、明示した歴史上の時点・期間、作成日・更新日、「関係」には意味関係と内部参照、「履歴」には別稿や版系統への入口があります。左ナビゲーションからQueryを開くと、同じペインがQuery・検索別名の表示に切り替わります。</li>
			</ul>
			<div class="help-actions">
		...
		<article class="help-card">
			...
			<h2>参照と意味関係</h2>
			<ul>
				<li>本文中で <kbd>[[</kbd> を入力すると既存の項目や版への候補が開き、選ぶと不変IDを使う「内部参照」（Work/Revisionリンク）が本文に挿入されます。</li>
				<li><code>[[source::TYPE::target]]</code>と本文に書いた場合は別形式の関係候補として解析されます。候補をクリックするとInspector「関係」で解決結果を確認できますが、それだけでは意味関係は作成されません。</li>
				<li>本文中で <kbd>@</kbd> を入力すると項目間の連想をつなぐ候補が開きます。これは内部参照とは別の意味関係を作る操作です。相手を選び、関係型と向きを確定すると意味関係が作成され、<kbd>@</kbd>の検索語は本文から取り除かれます。候補一覧で<kbd>Shift</kbd> + <kbd>Enter</kbd>を押すと、検索語を未配置項目として作成して相手に選べます。</li>
				<li>選択項目のインスペクター「関係」から、リンク元・種類・リンク先・向きを確認して保存できます。</li>
				<li>組み込みの関係型に加え、Optionの「意味関係の種類」から有向・双方向のカスタム関係型を追加できます。有向型は系統樹で世代を進める設定もできます。</li>
		...
		<article class="help-card">
			...
			<ul>
				...
				<li>インスペクター「概要」には作成日・更新日が表示され、「履歴」には別稿や版系統への入口があります。「全体系統」と「版系統」でも関係履歴を確認できます。</li>
				<li>TreeのChronologyでは「作成日時」と、インスペクター「概要」で設定した歴史上の時点・期間を使う「年表」を切り替えられます。これは「今日」ビューの作成・更新日とは別です。Lineageでは世代を進める設定の関係型に沿って表示します。</li>
			</ul>
		</article>
		...
		<article class="help-card help-card--wide">
			...
			{/if}
			<p class="help-note">コマンドパレットは <kbd>Ctrl</kbd> + <kbd>K</kbd> で開き、登録された操作を検索して実行できます。実行条件を満たさない操作は理由付きで無効になります。<kbd>Esc</kbd>で閉じると、元の入力位置へフォーカスが戻ります。</p>
		</article>
