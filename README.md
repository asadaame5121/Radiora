...
- ヘッダーのクイック入力は配置先を決めずに項目を作り、`未配置箱`で編集・タグ付けしてから
  明示的に配置できます。
- `Ctrl+K`で検索可能なコマンドパレットを開き、登録された操作を実行できます。前提を満たさない操作は理由付きで無効になり、`Esc`で元の入力位置へ戻ります。
...
- 長文の編集は選択項目のインスペクター「概要」にある`原稿として開く`から、中央ペインに展開して編集します。保存またはキャンセルでアウトラインに戻り、Markdownプレビューにも切り替えられます。
- インスペクターは「概要・関係・履歴」を切り替えます。「概要」には配置と作成日・更新日、「関係」には意味関係と内部参照、「履歴」には別稿や版系統への入口があります。左ナビゲーションのQueryを開くと、右ペインがQuery・検索別名の表示に切り替わります。
...
- 本文で`[[`を入力すると内部参照の候補が開き、項目か版を選ぶと不変IDを使う参照が本文に挿入されます。被参照一覧から参照元へ移動できます。
- 本文中の`[[source::TYPE::target]]`は別形式の関係候補として解析され、クリックするとInspectorの「関係」で解決結果を確認できます。確認だけでは関係を作成しません。
- 本文で`@`を入力するとOmniWindowで意味関係の相手を検索できます。相手を選んで関係種別と向きを確定すると意味関係が作成され、`@`の検索語は本文から取り除かれます。`Shift+Enter`では検索語を未配置項目として作成し、関係の相手に選べます。
- 組み込みの関係型に加え、Optionの「意味関係の種類」から有向・双方向のカスタム関係型を登録できます。有向型は系統樹の世代を進める設定もできます。
...
Treeビューでは`Outline / Tree`で切り替えます。`Chronology`では`作成日時`と、Inspector「概要」で設定した歴史上の時点・期間を使う`年表`を選びます。これは「今日」ビューの作成・更新日とは別のものです。
`Lineage`では世代を進める設定の関係型に沿って世代をX軸に表示します。その関係型の循環は世代計算から隔離され、右端の`Knot`列へ
