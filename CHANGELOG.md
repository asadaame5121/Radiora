...
- 項目へ歴史上の時点・期間を登録し、Chronologyを作成日時と歴史年表で切り替えて表示できるようにした。

## 0.5.2 - 2026-09-08

### Added

- Optionでユーザー定義の意味関係型を追加し、有向／対称と系統表示での世代前進を設定できるようにした。

### Fixed

- Outlineのdrag/dropで移動元を安定して保持し、保存中の選択・フォーカス復元、取消、画面離脱を修正。
- Lightテーマでフォーカス中のOutline本文がアプリの前景色に追従するよう修正。
