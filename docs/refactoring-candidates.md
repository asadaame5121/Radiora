...
週に1日をリファクタリング日にし、通常の機能開発とは専用の branch/worktree（Jujutsu では
専用 workspace/change）を分けて少しずつ進める。
...
最終的な責務は、アプリの bootstrap、feature 間の接続、トップレベル View の選択に限定する。
共有 Rune の state owner は `*.svelte.ts` の Controller/ViewModel に置く。
