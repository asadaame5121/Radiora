...
この計画は、画面設計・UI文言の再設計を対象外として、既存責務の抽出を優先した記録である。
WP0〜WP5の抽出結果は現行mainの基盤として維持されている一方、後続変更でアウトライン中心の
画面構成、中央ペインの長文編集モード、インスペクター、Markdown表示の操作仕様が更新された。
これらの後続UI変更は本計画の完了結果を書き換えるものではなく、当初の対象外に属する別変更
として扱う。
