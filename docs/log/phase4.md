...
## #51 [P4] Phase 4: 検索と知識整備 (phase, OPEN)
...
## #52 [P4-01] 検索・Today・Query共通の一時Occurrence投影モデルを実装する (task, area:search, OPEN)
...
## #53 [P4-02] Sparse Outline生成器を実装する (task, area:search, OPEN)
...
## #54 [P4-03] 保存Query結果をSparse Outline表示へ接続する (task, area:search, OPEN)
...
## #55 [P4-04] Stub状態・作成文脈・Backlink・一覧を実装する (task, area:domain, OPEN)
...
## #56 [P4-05] 重複候補のscoreと根拠表示を実装する (task, area:search, OPEN)
...
## #57 [P4-06] 重複統合previewとtransactional mergeを実装する (task, area:domain, OPEN)
...
## #58 [P4-07] 非統合時にLIKE・RELATEDを選ぶ経路を実装する (task, area:ui, OPEN)
...
## #59 [P4-08] 発見候補を確定リンクと別エンティティへ移す (task, area:search, OPEN)
...
## #60 [P4-09] 高密度データの性能・選択文脈テストを追加する (task, area:quality, OPEN)
